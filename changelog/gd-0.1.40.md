...
* **Manuelle Personalzuweisung:** POI-Personal sucht sich Arbeitsplätze nicht mehr unkontrolliert selbst. Zuweisungen erfolgen nun gezielt über die Personalverwaltung.
* **Neues Listen-UI:** Klobige Standard-Dropdowns wurden im Zuweisungs- und Einstellungs-Tab durch ein hochwertiges, ins Spiel-Design passendes `ListModal` ersetzt.
* **Verbessertes Laufverhalten (Gastronomie):** Bedienungen haben nun feste Ankerpunkte im Gastraum (via `WaiterArea` Node) und bewegen sich nur noch in einem kleinen Radius darum. Kein Umherwandern mehr hinter Tresen!
* **Blickrichtung:** Barkeeper richten sich hinter dem Tresen nun immer korrekt zur Tür hin aus.
...
* **Scroll-Bug behoben:** Die Personalliste springt beim Scrollen nicht mehr willkürlich nach oben zurück (behoben durch Anpassungen am Rendering der Progress-Bars).
...
* **Automatik entfernt:** Ausbau der automatischen POI-Slot-Füllroutine im `StaffManager`.
* **Fallback-Punkte:** Fallback-Punkte für Bedienungen in Räumen ohne expliziten `WaiterArea` Node hinzugefügt.
