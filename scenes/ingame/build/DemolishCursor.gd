...
func _try_demolish() -> void:
	...
	if room.get("room_type_id") == "lobby":
		if is_instance_valid(Toast):
			Toast.show(GameState.T("toast.room.lobby_no_demolish"), "build", false)
		return
		
	if room.get("is_pending_demolish"):
		if is_instance_valid(Toast):
			Toast.show(GameState.T("toast.room.already_demolish"), "build", false)
		return
	...
	if def.get("is_staff_poi", false) and def.get("capacity", 0) > 0:
		if StaffManager and not StaffManager.can_demolish_staff_room(def.get("capacity", 4)):
			if is_instance_valid(Toast):
				Toast.show(GameState.T("toast.room.staff_capacity_error", "Personal abbauen bevor abgerissen wird!"), "build", false)
			return
