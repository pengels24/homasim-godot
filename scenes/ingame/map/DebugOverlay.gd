...
func _draw() -> void:
    ...
    if map._debug_path.size() > 1:
        for i in range(map._debug_path.size() - 1):
            var p1 = Vector2((map._debug_path[i].x + map.WALK_W + 0.5) * map.TILE_PX, (map._debug_path[i].y + map.WALK_W + 0.5) * map.TILE_PX)
            var p2 = Vector2((map._debug_path[i+1].x + map.WALK_W + 0.5) * map.TILE_PX, (map._debug_path[i+1].y + map.WALK_W + 0.5) * map.TILE_PX)
            draw_line(p1, p2, Color(1, 1, 0, 0.8), 1.0)