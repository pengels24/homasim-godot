...
ame_key statt 
ame verwendet).
- **Kassenbuch Checkout-Details:** Checkout-Logs um Zimmertyp und Aufenthaltsdauer erweitert. ModalContentFinances.gd unterst�tzt nun beliebig viele Platzhalter in der �bersetzung.
...
- **GuestActor Navigation-Refactoring:** _get_logical_start_tile() komplett �berarbeitet und das Flickwerk hardcodierter Status-Checks (EATING, STUDYING_MENU etc.) entfernt. Start-Kachel-Ermittlung aus POIs funktioniert nun generisch und robust �ber _current_poi_id, was SOLID-Fehler bei unerwarteten Statuswechseln (z. B. Bettzeit um 23:00 Uhr w�hrend Wartezeit) verhindert.
