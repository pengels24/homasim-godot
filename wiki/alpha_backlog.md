...
- **Table Hopping (Smart Room Fix):** Timer-Check in `GuestActor.gd` (`_process_waiting`) ergänzt, um Endlosschleifen beim Reservieren von Plätzen (Sitzplatzwechsel pro Frame) zu verhindern.
- **Gastro-Room Bypass:** Gastro-Räume springen nun sofort in `STUDYING_MENU` und umgehen die fehleranfällige Smart-Room Loop. Legacy-Code (`claim_seat`) entfernt.
- **Bar Live-Monitor:** Zeigt nun "Studiert Speisekarte" und "Wird abgeholt & serviert" an (anstatt sofort auf "Sitzt / Trinkt" zu springen). Scope-Bug gefixt.
- **StaffActor Pathing & Orders:** `order_id` in `serve_meal`-Task wird korrekt ausgelesen, sodass Bestellungen im Live-Monitor den "serving" Status erhalten. Debug-Pfade zeigen nun Mitarbeiternamen statt IDs.
