...
*   **Kapazität (`max_guests`):** Limitiert auf exakt **8** Gäste (`Chair1` bis `Chair8`). Physisch wie programmatisch (seit `v0.1.50` auch als Property im Dictionary verankert). Findet ein Gast keinen freien `Chair` via `claim_interaction()`, bricht er den Besuch ab.
